...
$.getJSON("boilerplate-new.js", function(json) {
    console.log(json);
   ...
   for(var i=0;i<keepTags.length;i++){
    ...
    for(var i=0;i<keepVariables.length;i++){
        ...
        if(keepVariables[i].type == "jsm"){
            var text = JSON.stringify(keepVariables[i]);
            var result = text.match(reg);

            console.log("CHECKING: " + keepVariables[i].name);
...
function modifyObject(){
    ...
    modified = JSON.parse(JSON.stringify(obj));;
